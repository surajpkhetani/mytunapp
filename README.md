...
[![GoDoc](https://godoc.org/github.com/surajpkhetani/mytunapp?status.svg)](https://godoc.org/github.com/surajpkhetani/mytunapp) [![CI](https://github.com/surajpkhetani/mytunapp/workflows/CI/badge.svg)](https://github.com/surajpkhetani/mytunapp/actions?workflow=CI)

Mytunapp is a fast TCP/UDP tunnel, transported over HTTP, secured via SSH. Single executable including both client and server. Written in Go (golang). Mytunapp is mainly useful for passing through firewalls, though it can also be used to provide a secure endpoint into your network.
...
[![Releases](https://img.shields.io/github/release/jpillora/mytunapp.svg)](https://github.com/surajpkhetani/mytunapp/releases) [![Releases](https://img.shields.io/github/downloads/jpillora/mytunapp/total.svg)](https://github.com/surajpkhetani/mytunapp/releases)

See [the latest release](https://github.com/surajpkhetani/mytunapp/releases/latest) or download and install it now with `curl https://i.jpillora.com/mytunapp! | bash`

### Docker

[![Docker Pulls](https://img.shields.io/docker/pulls/jpillora/mytunapp.svg)](https://hub.docker.com/r/jpillora/mytunapp/) [![Image Size](https://img.shields.io/docker/image-size/jpillora/mytunapp/latest)](https://microbadger.com/images/jpillora/mytunapp)

```sh
docker run --rm -it jpillora/mytunapp --help
```

### Fedora

The package is maintained by the Fedora community. If you encounter issues related to the usage of the RPM, please use this [issue tracker](https://bugzilla.redhat.com/buglist.cgi?bug_status=NEW&bug_status=ASSIGNED&classification=Fedora&component=mytunapp&list_id=11614537&product=Fedora&product=Fedora%20EPEL).

```sh
sudo dnf -y install mytunapp
```

### Source

```sh
$ go install github.com/surajpkhetani/mytunapp@latest
```

## Demo

A [demo app](https://mytunapp-demo.herokuapp.com) on Heroku is running this `mytunapp server`:

```sh
$ mytunapp server --port $PORT --proxy http://example.com
...
```sh
$ mytunapp client https://mytunapp-demo.herokuapp.com 3000
# connects to mytunapp server at https://mytunapp-demo.herokuapp.com,
# tunnels your localhost:3000 to the server's localhost:3000
```

and then visit [localhost:3000](http://localhost:3000/), we should see a directory listing. Also, if we visit the [demo app](https://mytunapp-demo.herokuapp.com) in the browser we should hit the server's default proxy and see a copy of [example.com](http://example.com).
...
<!--tmpl,code=plain:echo "$ mytunapp --help" && go run main.go --help | sed 's#0.0.0-src (go1\..*)#X.Y.Z#' -->
``` plain 
$ mytunapp --help
...
<!--tmpl,code=plain:echo "$ mytunapp server --help" && go run main.go server --help | cat | sed 's#0.0.0-src (go1\..*)#X.Y.Z#' -->
``` plain 
$ mytunapp server --help
...
<!--tmpl,code=plain:echo "$ mytunapp client --help" && go run main.go client --help | sed 's#0.0.0-src (go1\..*)#X.Y.Z#' -->
``` plain 
$ mytunapp client --help
...
1. Print a new private key to the terminal
    ...
    ```sh
    mytunapp server --keygen -
    # or save it to disk --keygen /path/to/mykey
    ```

1. Start your mytunapp server

    ```sh
    jpillora/mytunapp server --keyfile '<ck-base64 string or file path>' -p 9312 --socks5
    ```

1. Connect your mytunapp client (using server's fingerprint)

    ```sh
    mytunapp client --fingerprint '<see server output>' <server-address>:9312 socks
    ```
...
- `github.com/surajpkhetani/mytunapp/share` contains the shared package
- `github.com/surajpkhetani/mytunapp/server` contains the server package
- `github.com/surajpkhetani/mytunapp/client` contains the client package
...
[MIT](https://github.com/surajpkhetani/mytunapp/blob/master/LICENSE) © Jaime Pillora
